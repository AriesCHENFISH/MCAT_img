...
class MCAT_Surv(nn.Module):
    ...
    def forward(self, x_ct, x_dsa):
        """
        x_ct: CT feature tensor (n_patches, 1, 512)
        x_dsa: DSA feature tensor (1, 512)
        """
        ...
        # --- CT feature ---
        h_ct = self.ct_fc(x_ct.squeeze(0))  # (n_patches, 512)
        print(f"h_ct shape after fc: {h_ct.shape}")

        # --- DSA feature ---
        h_dsa = self.dsa_fc(x_dsa.squeeze(0))  # (512,)
        print(f"h_dsa shape after fc: {h_dsa.shape}")


        # Reshape h_dsa: (seq_len=1, batch_size=1, embed_dim=512)
        # h_dsa = h_dsa.unsqueeze(0)  # (1, 1, 512)

        print(f"h_ct shape: {h_ct.shape}, h_dsa shape: {h_dsa.shape}")

        # --- Co-Attention ---
        h_ct2dsa, coattn_matrix = self.coattn(h_ct, h_dsa, h_dsa)  # (n_patches, 1, 512)
        h_ct = self.ct_norm(h_ct2dsa + h_ct)
        print(f"h_ct after coattn: {h_ct.shape}")

        # --- Transformer Encoding ---
        h_ct_trans = self.ct_transformer(h_ct.permute(1, 0, 2))  # (1, n_patches, 512)
        h_dsa_trans = self.dsa_transformer(h_dsa.permute(1, 0, 2))  # (1, 1, 512)
        print(f"h_ct_trans shape: {h_ct_trans.shape}, h_dsa_trans shape: {h_dsa_trans.shape}")

        # --- Attention Pooling ---
        ct_attention_weights = F.softmax(self.ct_attention(h_ct_trans), dim=1)  # (1, n_patches, 1)
        dsa_attention_weights = F.softmax(self.dsa_attention(h_dsa_trans), dim=1)  # (1, 1, 1)

        h_ct_final = torch.sum(ct_attention_weights * h_ct_trans, dim=1)  # (1, 512)
        h_dsa_final = torch.sum(dsa_attention_weights * h_dsa_trans, dim=1)  # (1, 512)

        print(f"h_ct_final shape: {h_ct_final.shape}, h_dsa_final shape: {h_dsa_final.shape}")

        # --- Fusion ---
        if self.fusion == 'bilinear':
            h_final = self.fusion_layer(h_ct_final, h_dsa_final)  # (1, 512)
        elif self.fusion == 'concat':
            h_final = self.fusion_layer(torch.cat([h_ct_final, h_dsa_final], dim=1))  # (1, 512)
        else:
            h_final = h_ct_final + h_dsa_final  # (1, 512)

        print(f"h_final shape: {h_final.shape}")

        # --- Classification ---
        logits = self.classifier(h_final)  # (1, 1)
