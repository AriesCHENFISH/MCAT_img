...
class MCAT_Surv(nn.Module):
    ...
    def forward(self, x_ct, x_dsa):
        """
        x_ct: CT feature tensor (batch_size, 512)
        x_dsa: DSA feature tensor (batch_size, 512)
        """
        # Bag-Level Representation
        h_ct = self.ct_fc(x_ct)  # (batch_size, 1, 512)
        h_dsa = self.dsa_fc(x_dsa)  # (batch_size, 1, 512)

        print(f"h_ct shape: {h_ct.shape}, h_dsa shape: {h_dsa.shape}")
        h_ct = h_ct.squeeze(1)
        h_dsa = h_dsa.squeeze(1)
        print(f"h_ct shape: {h_ct.shape}, h_dsa shape: {h_dsa.shape}")

        # Ensure the shapes of h_ct and h_dsa are [seq_len, batch_size, embed_dim]
        if h_dsa.dim() == 2:  # [batch_size, embed_dim]
            h_dsa = h_dsa.unsqueeze(0)  # [1, batch_size, embed_dim]

        if h_ct.dim() == 3:  # [batch_size, N, embed_dim]
            h_ct = h_ct.permute(1, 0, 2)  # [seq_len=1, batch_size, embed_dim]
        elif h_ct.dim() == 4:  # [batch_size, 1, N, embed_dim]
            h_ct = h_ct.squeeze(1).permute(1, 0, 2)  # [seq_len=1, batch_size, embed_dim]

        print(f"h_ct shape after permute: {h_ct.shape}, h_dsa shape after permute: {h_dsa.shape}")

        # Co-Attention
        h_ct_coattn, coattn_matrix = self.coattn(h_dsa, h_ct, h_ct)  # (seq_len, batch_size, embed_dim), (seq_len, batch_size, 1)
        print(f"h_ct_coattn shape: {h_ct_coattn.shape}, coattn_matrix shape: {coattn_matrix.shape}")

        # Adjust back to original batch-first shape
        h_ct_coattn = h_ct_coattn.permute(1, 0, 2)  # (batch_size, seq_len, embed_dim)

        # Set-Based Transformer
        h_ct_trans = self.ct_transformer(h_ct_coattn)  # (batch_size, seq_len, embed_dim)
        h_dsa_trans = self.dsa_transformer(h_dsa)  # (batch_size, seq_len, embed_dim)

        # Attention Pooling
        ct_attention_weights = F.softmax(self.ct_attention(h_ct_trans), dim=1)  # (batch_size, seq_len, 1)
        dsa_attention_weights = F.softmax(self.dsa_attention(h_dsa_trans), dim=1)  # (batch_size, seq_len, 1)

        h_ct_final = torch.sum(ct_attention_weights * h_ct_trans, dim=1)  # (batch_size, embed_dim)
        h_dsa_final = torch.sum(dsa_attention_weights * h_dsa_trans, dim=1)  # (batch_size, embed_dim)

        print(f"h_ct_final shape: {h_ct_final.shape}, h_dsa_final shape: {h_dsa_final.shape}")

        # Fusion
        if self.fusion == 'bilinear':
            h_final = self.fusion_layer(h_ct_final, h_dsa_final)  # (batch_size, embed_dim)
        elif self.fusion == 'concat':
            h_dsa_final = h_dsa_final.expand(h_ct_final.size(0), -1)  # Expand to (batch_size, embed_dim)
            h_final = self.fusion_layer(torch.cat([h_ct_final, h_dsa_final], dim=1))  # (batch_size, embed_dim)
        else:
            h_final = h_ct_final + h_dsa_final  # Simple Addition (batch_size, embed_dim)
