...
class MCAT_Surv(nn.Module):
    ...
    def forward(self, x_ct, x_dsa):
        ...
        # 保证 h_dsa 是三维 [seq_len, B, embed_dim]
        if h_dsa.dim() == 2:
            h_dsa = h_dsa.unsqueeze(1)  # [1, B, C]
        elif h_dsa.dim() == 1:
            h_dsa = h_dsa.unsqueeze(0).unsqueeze(0)  # [1, 1, C]

        print(f"h_ct shape: {h_ct.shape}, h_dsa shape: {h_dsa.shape}")


        ### **调整形状以匹配 MultiheadAttention**
        h_ct = h_ct.permute(1, 0, 2)  # (1, batch_size, 512) -> (seq_len=1, batch_size, 512)
        h_dsa = h_dsa.permute(1, 0, 2)  # (1, batch_size, 512) -> (seq_len=1, batch_size, 512)

        # print(f"After permute:")
        # print(f"h_ct shape: {h_ct.shape}, h_dsa shape: {h_dsa.shape}")

        ### **Co-Attention**
        h_ct_coattn, coattn_matrix = self.coattn(h_dsa, h_ct, h_ct)  # (1, batch_size, 512), (1, batch_size, 1)
        # print(f"h_ct_coattn shape: {h_ct_coattn.shape}, coattn_matrix shape: {coattn_matrix.shape}")

        ### **调整回原来的 batch-first 形状**
        h_ct_coattn = h_ct_coattn.permute(1, 0, 2)  # (batch_size, 1, 512)

        ### **Set-Based Transformer**
        h_ct_trans = self.ct_transformer(h_ct_coattn)  # (batch_size, 1, 512)
        h_dsa_trans = self.dsa_transformer(h_dsa)  # (batch_size, 1, 512)

        ### **Attention Pooling**
        ct_attention_weights = F.softmax(self.ct_attention(h_ct_trans), dim=1)  # (batch_size, 1, 1)
        dsa_attention_weights = F.softmax(self.dsa_attention(h_dsa_trans), dim=1)  # (batch_size, 1, 1)

        h_ct_final = torch.sum(ct_attention_weights * h_ct_trans, dim=1)  # (batch_size, 512)
        h_dsa_final = torch.sum(dsa_attention_weights * h_dsa_trans, dim=1)  # (batch_size, 512)
        print(f"h_ct_final shape: {h_ct_final.shape}, h_dsa_final shape: {h_dsa_final.shape}")

        # Fusion
        if self.fusion == 'bilinear':
            h_final = self.fusion_layer(h_ct_final, h_dsa_final)  # (batch_size, 512)
        elif self.fusion == 'concat':
            h_dsa_final = h_dsa_final.expand(h_ct_final.size(0), -1)  # 扩展为 (batch_size, 512)
            print("h_dsa_final_now:",h_dsa_final.shape)
            h_final = self.fusion_layer(torch.cat([h_ct_final, h_dsa_final], dim=1))  # (batch_size, 512)
            print("h_final_now:",h_final.shape)
        else:
            h_final = h_ct_final + h_dsa_final  # Simple Addition (batch_size, 512)
